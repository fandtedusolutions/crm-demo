...
use App\Support\HospitalAdministrationMentorTrackColumns;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Validator;

class HospitalAdministrationMentorController extends Controller
{
    private const COURSE_ID = 4;
    ...
    public function index(Request $request)
    {
        ...
        $activeMentorRoute = 'admin.hospital-administration-mentor-converted-leads.index';

        return view('admin.converted-leads.hospital-administration-mentor-index', compact(
            'activeMentorRoute',
            'convertedLeads',
            'batches',
            'course',
    ...
    public function updateMentorDetails(Request $request, $id)
    {
        try {
            ...
            $rule = HospitalAdministrationMentorTrackColumns::validationRuleFor($field);
            if ($rule) {
            ...
            $mentorDetailFields = HospitalAdministrationMentorTrackColumns::mentorDetailFields();
            ...
            HospitalAdministrationMentorTrackColumns::setTrackValue(
                $mentorDetails,
                $field,
                ($value === '' ? null : $value)
            );
            ...
            $responseValue = $value;
            if (in_array($field, HospitalAdministrationMentorTrackColumns::dateFields(), true) && $value) {
            ...
            Log::error('Hospital Administration mentor update error: '.$e->getMessage());
