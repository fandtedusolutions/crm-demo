...
abstract class BaseGraphicDesigningStyleRegistrationController extends Controller
{
    ...
    abstract protected function storageKey(): string;
    ...
    public function showForm($leadId = null)
    {
        ...
        if ($leadId) {
            ...
            if ($lead) {
                ...
                if ($studentDetail) {
                    return view('public.graphic-designing-style-registration-success', [
                        'registrationTitle' => $this->courseTitle(),
                    ]);
                }
            }
        }
        ...
        $subjects = Subject::where('course_id', $courseId)->where('is_active', true)->get();
        $batches = Batch::where('course_id', $courseId)->where('is_active', true)->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            $fileFields = [
                'passport_photo', 'adhar_front', 'adhar_back', 'signature', 'sslc_certificate',
                'plus_two_certificate', 'graduation_certificate', 'post_graduation_certificate', 'other_relevant_documents',
            ];
            ...
            try {
                ...
                \Log::error('Email sending failed for ' . $this->courseTitle() . ' registration: ' . $e->getMessage());
            }
            ...
            return response()->json([
                ...
                'data' => $studentDetail,
                'redirect' => route($this->registerRouteName(), $request->lead_id),
            ]);
        } catch (\Exception $e) {
