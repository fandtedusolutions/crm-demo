@php
    ...
    if (!empty($filters['telecaller_id']) && isset($telecallers)) {
        $tc = $telecallers->firstWhere('id', (int) $filters['telecaller_id']);
        if ($tc) {
            $chips[] = ['label' => 'Telecaller', 'value' => $tc->name];
        }
    }
    ...
    $hasExtraFilters = !empty($filters['telecaller_id']) || !empty($filters['call_type']) || !empty($filters['search']) || !$isDefaultDate;
@endphp
