        ...
        <div class="row align-items-center">
            <div class="col-md-6">
                <div class="page-header-title">
                    <h5 class="m-b-10">Robo Vibe - Course Support List</h5>
                </div>
            </div>
            <div class="col-md-6">
                <div class="d-flex justify-content-end align-items-center gap-3">
                    <ul class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item">Robo Vibe Support</li>
                    </ul>
...
<div class="row mb-3">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form method="GET" action="{{ route('admin.support-robo-vibe-converted-leads.index') }}" id="filterForm">
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-12 col-sm-6 col-md-2">
                            ...
                            <a href="{{ route('admin.support-robo-vibe-converted-leads.index') }}" class="btn btn-secondary"><i class="ti ti-refresh"></i> Clear</a>
                        </div>
                    </div>
                </form>
            </div>
                ...
                <div class="d-none d-lg-block">
                    <div class="table-responsive">
                        ...
                        <tbody>
                            ...
                            @php
                                $rv = $lead->lead ? $lead->lead->roboVibeStudentDetails : null;
                            ...
                            <tr class="{{ $lead->is_cancelled ? 'cancelled-row' : '' }}">
                                ...
                                <td>
                                    @if($canEdit)
                                    <div class="inline-edit" data-field="whatsapp_number" data-id="{{ $lead->id }}" data-current="{{ $rv?->whatsapp_number }}">
                                        <span class="display-value">{{ $rv && $rv->whatsapp_number ? \App\Helpers\PhoneNumberHelper::display($rv->whatsapp_code, $rv->whatsapp_number) : '-' }}</span>
                                        <button class="btn btn-sm btn-outline-secondary ms-1 edit-btn" title="Edit"><i class="ti ti-edit"></i></button>
                                    </div>
                                    @else
                                    {{ $rv && $rv->whatsapp_number ? \App\Helpers\PhoneNumberHelper::display($rv->whatsapp_code, $rv->whatsapp_number) : '-' }}
                                    @endif
                                </td>
                                <td>
                                    @if($canEdit)
                                    <div class="inline-edit" data-field="parents_number" data-id="{{ $lead->id }}" data-current="{{ $rv?->parents_number }}">
                                        <span class="display-value">{{ $rv && $rv->parents_number ? \App\Helpers\PhoneNumberHelper::display($rv->parents_code, $rv->parents_number) : '-' }}</span>
                                        <button class="btn btn-sm btn-outline-secondary ms-1 edit-btn" title="Edit"><i class="ti ti-edit"></i></button>
                                    </div>
                                    @else
                                    {{ $rv && $rv->parents_number ? \App\Helpers\PhoneNumberHelper::display($rv->parents_code, $rv->parents_number) : '-' }}
                                    @endif
                                </td>
                                <td>
                                    @if($canEdit)
                                ...
                                <td>
                                    @if($canEdit)
                                    <div class="inline-edit" data-field="batch_id" data-id="{{ $lead->id }}" data-course-id="33" data-current-id="{{ $lead->batch_id }}">
                                ...
                                <td>
                                    ...
                                    <div class="inline-edit" data-field="class_time_id" data-id="{{ $lead->id }}" data-course-id="33" data-programme-type="{{ $rv?->programme_type ?? 'online' }}" data-current-id="{{ $rv?->class_time_id }}">
                                        <span class="display-value">
                                            @if($rv && $rv->classTime)
                                            {{ \Carbon\Carbon::parse($rv->classTime->from_time)->format('h:i A') }} - {{ \Carbon\Carbon::parse($rv->classTime->to_time)->format('h:i A') }}
                                            @else - @endif
                                        </span>
                                        <button class="btn btn-sm btn-outline-secondary ms-1 edit-btn" title="Edit"><i class="ti ti-edit"></i></button>
                                    </div>
...
@include('admin.converted-leads.partials.support-flag-inline-scripts', ['supportUpdateUrl' => route('admin.support-robo-vibe-converted-leads.update-support-details', ['id' => '__ID__'])])
...
$(document).ready(function() {
    ...
    var updateUrl = '{{ route("admin.support-robo-vibe-converted-leads.update-support-details", ":id") }}';
    ...
    $(document).on('click', '.edit-btn', function(e) {
        ...
        if (field === 'batch_id') {
            ...
            var courseId = $c.data('course-id') || 33;
