...
@section('title', 'Prompt Engineering - Course Support List')
        ...
        <div class="row align-items-center">
            <div class="col-md-6">
                <div class="page-header-title">
                    <h5 class="m-b-10">Prompt Engineering - Course Support List</h5>
                </div>
            </div>
            <div class="col-md-6">
                <div class="d-flex justify-content-end align-items-center gap-3">
                    <ul class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item">Prompt Engineering Support</li>
                    </ul>
...
<div class="row mb-3">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form method="GET" action="{{ route('admin.support-prompt-engineering-converted-leads.index') }}" id="filterForm">
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-12 col-sm-6 col-md-2">
                            ...
                            <a href="{{ route('admin.support-prompt-engineering-converted-leads.index') }}" class="btn btn-secondary"><i class="ti ti-refresh"></i> Clear</a>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Prompt Engineering - Course Support List</h5>
                @include('admin.converted-leads.partials.export-buttons', ['exportPage' => 'support-prompt-engineering'])
            </div>
            <div class="card-body">
                @php
                    ...
                    $course = \App\Models\Course::find(34);
                @endphp
                <!-- Desktop Table View -->
                <div class="d-none d-lg-block">
                    <div class="table-responsive">
                        ...
                        <tbody>
                            ...
                            @php
                                $pe = $lead->lead ? $lead->lead->promptEngineeringStudentDetails : null;
                            ...
                            <tr class="{{ $lead->is_cancelled ? 'cancelled-row' : '' }}">
                                ...
                                <td>
                                    @if($canEdit)
                                    <div class="inline-edit" data-field="whatsapp_number" data-id="{{ $lead->id }}" data-current="{{ $pe?->whatsapp_number }}">
                                        <span class="display-value">{{ $pe && $pe->whatsapp_number ? \App\Helpers\PhoneNumberHelper::display($pe->whatsapp_code, $pe->whatsapp_number) : '-' }}</span>
                                        <button class="btn btn-sm btn-outline-secondary ms-1 edit-btn" title="Edit"><i class="ti ti-edit"></i></button>
                                    </div>
                                    @else
                                    {{ $pe && $pe->whatsapp_number ? \App\Helpers\PhoneNumberHelper::display($pe->whatsapp_code, $pe->whatsapp_number) : '-' }}
                                    @endif
                                </td>
                                <td>
                                    @if($canEdit)
                                    <div class="inline-edit" data-field="parents_number" data-id="{{ $lead->id }}" data-current="{{ $pe?->parents_number }}">
                                        <span class="display-value">{{ $pe && $pe->parents_number ? \App\Helpers\PhoneNumberHelper::display($pe->parents_code, $pe->parents_number) : '-' }}</span>
                                        <button class="btn btn-sm btn-outline-secondary ms-1 edit-btn" title="Edit"><i class="ti ti-edit"></i></button>
                                    </div>
                                    @else
                                    {{ $pe && $pe->parents_number ? \App\Helpers\PhoneNumberHelper::display($pe->parents_code, $pe->parents_number) : '-' }}
                                    @endif
                                </td>
                                <td>
                                    @if($canEdit)
                                ...
                                <td>
                                    ...
                                    <div class="inline-edit" data-field="class_time_id" data-id="{{ $lead->id }}" data-course-id="34" data-programme-type="{{ $pe?->programme_type ?? 'online' }}" data-current-id="{{ $pe?->class_time_id }}">
                                        <span class="display-value">
                                            @if($pe && $pe->classTime)
                                            {{ \Carbon\Carbon::parse($pe->classTime->from_time)->format('h:i A') }} - {{ \Carbon\Carbon::parse($pe->classTime->to_time)->format('h:i A') }}
                                            @else - @endif
                                        </span>
                                        <button class="btn btn-sm btn-outline-secondary ms-1 edit-btn" title="Edit"><i class="ti ti-edit"></i></button>
                                    </div>
                                ...
                                <td>
                                    @if($canEdit)
                                    <div class="inline-edit" data-field="app" data-id="{{ $lead->id }}" data-field-type="select" data-current="{{ $lead->supportDetails?->app }}">
                                        <span class="display-value">{{ $lead->supportDetails?->app ?? '-' }}</span>
                                        <button class="btn btn-sm btn-outline-secondary ms-1 edit-btn" title="Edit"><i class="ti ti-edit"></i></button>
                                    </div>
                                    @else
                                    {{ $lead->supportDetails?->app ?? '-' }}
                                    @endif
                                </td>
                                <td>
                                    @if($canEdit)
...
@include('admin.converted-leads.partials.support-flag-inline-scripts', ['supportUpdateUrl' => route('admin.support-prompt-engineering-converted-leads.update-support-details', ['id' => '__ID__'])])
...
$(document).ready(function() {
    ...
    var updateUrl = '{{ route("admin.support-prompt-engineering-converted-leads.update-support-details", ":id") }}';
    ...
    $(document).on('click', '.edit-btn', function(e) {
        ...
        if (field === 'batch_id') {
            ...
            var courseId = $c.data('course-id') || 34;
