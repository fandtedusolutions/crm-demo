...
@section('title', 'Prompt Engineering Converted Faculty List')
        ...
        <div class="row align-items-center">
            <div class="col-md-6">
                <div class="page-header-title">
                    <h5 class="m-b-10">Prompt Engineering Converted Faculty List</h5>
                </div>
            </div>
            <div class="col-md-6">
                <ul class="breadcrumb d-flex justify-content-end">
                    ...
                    <li class="breadcrumb-item">Prompt Engineering Faculty</li>
                </ul>
            </div>
        </div>
    </div>
...
<div class="row mb-3">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form method="GET" action="{{ route('admin.prompt-engineering-faculty-converted-leads.index') }}" id="filterForm">
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-12 col-sm-6 col-md-2">
                            ...
                            <a href="{{ route('admin.prompt-engineering-faculty-converted-leads.index') }}" class="btn btn-secondary"><i class="ti ti-x"></i> Clear</a>
                        </div>
                    </div>
                </form>
            </div>
...
<div class="row">
    <div class="col-12">
        <div class="card" data-mentor-update-url="{{ route('admin.prompt-engineering-faculty-converted-leads.update-mentor-details', ['id' => '__ID__']) }}">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Prompt Engineering Converted Faculty List</h5>
                @include('admin.converted-leads.partials.export-buttons', ['exportPage' => 'faculty-prompt-engineering'])
            </div>
            <div class="card-body">
                <!-- Desktop Table View -->
                <div class="d-none d-lg-block">
                    <div class="table-responsive">
                        <table class="table table-hover data_table_basic" id="jvMentorTable">
                            @include('admin.converted-leads.partials.prompt-engineering-faculty-table', [
                                'canEdit' => $canEdit,
...
$(document).ready(function() {
    ...
    var updateUrlBase = '{{ route("admin.prompt-engineering-faculty-converted-leads.update-mentor-details", ":id") }}';
    ...
    function createNumberInput(currentVal, min, max) {
        var s = (currentVal === undefined || currentVal === null) ? '' : String(currentVal);
        var v = (s === '-' || s === '') ? '' : s;
        return '<div class="edit-form"><input type="number" class="form-control form-control-sm" min="' + min + '" max="' + max + '" step="1" value="' + v.replace(/"/g, '&quot;') + '"><div class="btn-group mt-1"><button type="button" class="btn btn-success btn-sm save-edit">Save</button><button type="button" class="btn btn-secondary btn-sm cancel-edit">Cancel</button></div></div>';
    }
    ...
    $(document).on('click', '.edit-btn', function(e) {
        ...
        if (field === 'batch_id') {
            ...
            $c.addClass('editing').append(html);
            $c.find('select').focus();
        } else if ($c.data('field-type') === 'number') {
            html = createNumberInput(current, $c.data('min'), $c.data('max'));
            $c.addClass('editing').append(html);
            $c.find('input').focus();
    ...
    $(document).on('click', '.save-edit', function(e) {
        ...
        if ($c.data('field-type') === 'number' && value !== '') {
            var num = parseInt(value, 10);
            var min = parseInt($c.data('min'), 10);
            var max = parseInt($c.data('max'), 10);
            if (isNaN(num) || num < min || num > max) {
                if (typeof toast_error === 'function') toast_error('Enter a number between ' + min + ' and ' + max);
                return;
            }
            value = String(num);
        }
...
@include('admin.converted-leads.partials.course-flag-inline-scripts', ['courseUpdateUrl' => route('admin.prompt-engineering-faculty-converted-leads.update-mentor-details', ['id' => '__ID__'])])
@endpush
