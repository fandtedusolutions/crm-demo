...
@section('title', 'Robo Vibe Converted Faculty List')
        ...
        <div class="row align-items-center">
            <div class="col-md-6">
                <div class="page-header-title">
                    <h5 class="m-b-10">Robo Vibe Converted Faculty List</h5>
                </div>
            </div>
            <div class="col-md-6">
                <ul class="breadcrumb d-flex justify-content-end">
                    ...
                    <li class="breadcrumb-item">Robo Vibe Faculty</li>
                </ul>
            </div>
        </div>
    </div>
...
<div class="row mb-3">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form method="GET" action="{{ route('admin.robo-vibe-faculty-converted-leads.index') }}" id="filterForm">
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-12 col-sm-6 col-md-2">
                            ...
                            <a href="{{ route('admin.robo-vibe-faculty-converted-leads.index') }}" class="btn btn-secondary"><i class="ti ti-x"></i> Clear</a>
                        </div>
                    </div>
                </form>
            </div>
...
<div class="row">
    <div class="col-12">
        <div class="card" data-mentor-update-url="{{ route('admin.robo-vibe-faculty-converted-leads.update-mentor-details', ['id' => '__ID__']) }}">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Robo Vibe Converted Faculty List</h5>
                @include('admin.converted-leads.partials.export-buttons', ['exportPage' => 'faculty-robo-vibe'])
            </div>
            <div class="card-body">
                <!-- Desktop Table View -->
                <div class="d-none d-lg-block">
                    <div class="table-responsive">
                        ...
                        <tbody>
                            ...
                            @php
                                $rvLead = $lead->lead ? $lead->lead->roboVibeStudentDetails : null;
                            ...
                            <tr class="{{ $lead->is_cancelled ? 'cancelled-row' : '' }}">
                                ...
                                <td>
                                    ...
                                    <div class="inline-edit" data-field="class_time_id" data-id="{{ $lead->id }}" data-course-id="{{ $lead->course_id }}" data-programme-type="{{ $rvLead?->programme_type }}" data-current-id="{{ $rvLead?->class_time_id }}">
                                        <span class="display-value">
                                            @if($rvLead && $rvLead->classTime)
                                            {{ \Carbon\Carbon::parse($rvLead->classTime->from_time)->format('h:i A') }} - {{ \Carbon\Carbon::parse($rvLead->classTime->to_time)->format('h:i A') }}
                                            @else - @endif
                                        </span>
                                        <button class="btn btn-sm btn-outline-secondary ms-1 edit-btn" title="Edit"><i class="ti ti-edit"></i></button>
                                    </div>
                                ...
                                @include('admin.converted-leads.partials.inline-app-status-cell', ['lead' => $lead, 'canEdit' => $canEdit, 'context' => 'mentor'])
                                <td>
                                    @if($canEdit)
                                ...
                                <td>
                                    @if($canEdit)
                                    <div class="inline-edit" data-field="whatsapp_number" data-id="{{ $lead->id }}" data-current="{{ $rvLead?->whatsapp_number }}">
                                        <span class="display-value">{{ $rvLead && $rvLead->whatsapp_number ? \App\Helpers\PhoneNumberHelper::display($rvLead->whatsapp_code, $rvLead->whatsapp_number) : '-' }}</span>
                                        <button class="btn btn-sm btn-outline-secondary ms-1 edit-btn" title="Edit"><i class="ti ti-edit"></i></button>
                                    </div>
                                    @else
                                    {{ $rvLead && $rvLead->whatsapp_number ? \App\Helpers\PhoneNumberHelper::display($rvLead->whatsapp_code, $rvLead->whatsapp_number) : '-' }}
                                    @endif
                                </td>
                                <td>
                                    @if($canEdit)
                                    <div class="inline-edit" data-field="medium_of_study" data-id="{{ $lead->id }}" data-current="{{ $rvLead?->medium_of_study }}">
                                        <span class="display-value">{{ $rvLead && $rvLead->medium_of_study ? ucfirst(str_replace('_', ' ', $rvLead->medium_of_study)) : '-' }}</span>
                                        <button class="btn btn-sm btn-outline-secondary ms-1 edit-btn" title="Edit"><i class="ti ti-edit"></i></button>
                                    </div>
                                    @else
                                    {{ $rvLead && $rvLead->medium_of_study ? ucfirst(str_replace('_', ' ', $rvLead->medium_of_study)) : '-' }}
                                    @endif
                                </td>
                                <td>
                                    @if($canEdit)
                                    <div class="inline-edit" data-field="previous_qualification" data-id="{{ $lead->id }}" data-current="{{ $rvLead?->previous_qualification }}">
                                        <span class="display-value">{{ $rvLead && $rvLead->previous_qualification ? ucfirst(str_replace('_', ' ', $rvLead->previous_qualification)) : '-' }}</span>
                                        <button class="btn btn-sm btn-outline-secondary ms-1 edit-btn" title="Edit"><i class="ti ti-edit"></i></button>
                                    </div>
                                    @else
                                    {{ $rvLead && $rvLead->previous_qualification ? ucfirst(str_replace('_', ' ', $rvLead->previous_qualification)) : '-' }}
                                    @endif
                                </td>
                                <td>
                                    @if($canEdit)
                                    <div class="inline-edit" data-field="technology_performance_category" data-id="{{ $lead->id }}" data-current="{{ $rvLead?->technology_performance_category }}">
                                        <span class="display-value">{{ $rvLead && $rvLead->technology_performance_category ? ucfirst(str_replace('_', ' ', $rvLead->technology_performance_category)) : '-' }}</span>
                                        <button class="btn btn-sm btn-outline-secondary ms-1 edit-btn" title="Edit"><i class="ti ti-edit"></i></button>
                                    </div>
                                    @else
                                    {{ $rvLead && $rvLead->technology_performance_category ? ucfirst(str_replace('_', ' ', $rvLead->technology_performance_category)) : '-' }}
                                    @endif
                                </td>
                                {{-- First term --}}
                                <td>
...
$(document).ready(function() {
    ...
    var updateUrlBase = '{{ route("admin.robo-vibe-faculty-converted-leads.update-mentor-details", ":id") }}';
    ...
    $(document).on('click', '.edit-btn', function(e) {
        ...
        if (field === 'batch_id') {
            ...
            html = createSelectAppStatus(current);
            $c.addClass('editing').append(html);
            $c.find('select').focus();
...
@include('admin.converted-leads.partials.course-flag-inline-scripts', ['courseUpdateUrl' => route('admin.robo-vibe-faculty-converted-leads.update-mentor-details', ['id' => '__ID__'])])
@include('admin.converted-leads.partials.inline-app-status-scripts')
@endpush
